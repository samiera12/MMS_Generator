...
* **Initial and Boundary Conditions (`0/` directory):** The `0/` directory will be having the files generated by `0_field.mako`(e.g., `0/U`, `0/p`). These files contain the `codedFixedValue` and `codedMixed` boundaries that mathematically map the exact solution to your grid patches.The patches like `front` and `back` with `empty` boundary condition, can be manually added.
* **Source Term Injection (`system/fvOptions`):** The `fvOptions` file will be generated in your `system/` directory.
* **Error Tracking (`system/controlDict`):** The generated function object is automatically added to your `system/` directory. This can then be included in `controlDict` using '#include`. This will compile at runtime and output the $L_{1}$, $L_{2}$, and $L_{\infty}$ norms to your terminal and log files at every timestep.
