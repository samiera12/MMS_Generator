...
### 3. Changes to Your OpenFOAM Case Files

To utilize the generated files, you must map them to your active OpenFOAM case directory.

* **Initial and Boundary Conditions (`0/` directory):** Replace your default initialization files (e.g., `0/U`, `0/p`) with the files generated by `0_field.mako`. These files contain the `codedFixedValue` and `codedMixed` boundaries that mathematically map the exact solution to your grid patches.
* **Source Term Injection (`system/fvOptions`):** Place the generated `fvOptions` file into your `system/` directory. You must ensure that your solver is configured to read `fvOptions` so it can inject the synthesized source term ($S$) into the governing matrix equations at runtime.
* **Error Tracking (`system/controlDict`):** Add the generated function object to your `system/controlDict` file (either directly or via an `#include` directive). This will compile at runtime and output the $L_{1}$, $L_{2}$, and $L_{\infty}$ norms to your terminal and log files at every timestep.
